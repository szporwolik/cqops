...
- **Rig menu stray blank lines**: the Edit Rig form rendered an empty line after "Radio control:" and "Rotator control:" when the backend was None — the section separator was written even when the sub-options block (host/port/poll) was hidden. The separator now belongs to the block, so collapsed rows sit flush while expanded rows keep their spacing. The same audit found the contest form's spacing shifted with its prefill toggles (a spacer lived outside the conditional block); its layout is now identical in every toggle state, with exactly one blank line before the marker reference section.
