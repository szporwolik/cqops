...
- Preparation and normalization leases now have a global owner: a preparation result that arrives after the operator left the editor screen was dropped without releasing its transferred lease, and normalization errors carried the lease but never entered the successful-normalization branch that cleans it up — a later logbook switch retained the retired database forever. The model now releases the lease globally for preparation results that cannot reach the editor handler and for normalization results whose follow-up will not run (errors or a dropped result).
- A batch upload failure no longer discards reconciliation work already identified: the all-remaining-uploads-failed result omitted the accumulated reconciliation ids, so a contact matched remotely with a newer local revision never got its required PATCH after the other uploads failed. Reconciliation ids now survive every return after the reconciliation pass, including failure results.
- The Abort button works again during downloads, imports and exports: the global key handler swallowed every key except F10 while an operation was active, so Enter and Escape never reached the operation's abort dialog and the operation kept running. Screen switching stays blocked, but the keys the active operation's dialog handles (Enter, Escape, Tab, arrows, q) now pass through to it.
- Download/import/export results can no longer be lost to extra channel readers: every tick dispatched another `readDownloadMsg`, so a second blocked reader received the channel-close zero value and its empty completion could finalize the operation with a zero counter before the real result arrived (the test reproduced `0` instead of `42`; errors could be lost the same way). Exactly one read may now be pending per operation, messages are bound to the operation's id (stale results from a replaced operation are discarded), and a channel close without a terminal message finalizes honestly with the captured error instead of fabricating a zero-count success.
- A normalization result can no longer leave an unreleased database lease when a download is active: the editor treated nearly every message as download progress while an operation ran, so a late normalization completion (e.g. after the editor was recreated with F8 and a download started) was swallowed before its handling — and a later logbook switch retained the retired database forever. Normalization completions are now distinguished and handled before the download progress branches; a discarded result releases its transferred lease (generation mismatch, list error), and the global handler releases every result the editor cannot consume.
- Download results can no longer be overtaken by channel closure: the pending-read flag was cleared in the reader goroutine immediately after reading, so a tick in the gap before `Update` could dispatch a second reader that observed the closure — its close-only completion could finalize the operation with zero ("operation did not finish") and the real terminal was then rejected as stale. The read now stays pending until the owner loop accepts the matching result (it is cleared there, and the next read is scheduled only after a nonterminal message), so the closure can never overtake the real terminal.
- Cancelling the quit dialog no longer cuts off access to a running operation: F10 switched the screen to the QSO form while showing the exit dialog, so after Escape the download/import/export kept running off-screen — navigation keys stayed blocked and Escape could no longer reach the Abort button. The quit dialog now renders over the source screen without switching it, so cancelling simply restores access to the running operation.
- Saving the form can no longer wipe the Wavelog id: when an upload assigned the remote id while the contact form was open, the form's stale snapshot (WavelogID 0) was written back over the database on save — the contact lost its Wavelog link (77 → 0, dirty=false) and the newer edit never triggered a PATCH. Field saves no longer write synchronization metadata, the synced/dirty decision is now made from the database state atomically with the write, and the follow-up PATCH targets the fresh row's remote id.
- Bulk pending-sync recovery: failed PATCHes and offline saves used to require re-saving each contact individually. The logbook editor now shows **Retry pending sync (Alt+P)** — the backlog is counted from the database (contacts with a remote id and a pending local edit), and confirming rebuilds the queue from the database and routes every contact through the shared per-contact serialization coordinator (one retry at a time, up to 50 rows per trigger), reusing the shared per-contact sync unit. The queue is derived from the store on every trigger, so edits pending across a restart are recovered.
- The bulk pending-sync retry no longer bypasses PATCH serialization: it initially sent its PATCHes outside the per-contact coordinator, so a delayed retry could overwrite the server with older content after a newer save completed first (`remote=old`, `local=new`, `dirty=false`). Every retry PATCH now goes through the same logbook/contact/endpoint coordinator as save chains — contacts whose save chain is already in flight are skipped as covered, a save made during a retry queues behind it and its follow-up pushes the newest revision, and the batch reports one summary toast when the last contact finishes.
- The bulk pending-sync summary no longer hides failed local acknowledgements: when the server accepted a PATCH but SQLite rejected clearing the pending flag, the completion was neither synced, failed, nor errored, so the aggregation reported **no pending changes** even though the contact still awaited synchronization. The batch tally now counts such contacts as unconfirmed — the summary warns with the synced/failed/unconfirmed counters instead of claiming success — and the row stays durably pending so the next Alt+P retry can confirm it.
- An empty pending-sync retry result no longer leaks the database lease: when a PATCH in flight synchronized the last pending change before the retry read the list, the empty result carried the transferred database lease but the global handler only consumed retry results with at least one contact — a later logbook switch then retained the retired database forever. Every retry result is now handled globally (empty or not): the handler consumes the lease and reports **no pending changes** honestly.
- **Logbook editor search is debounced and asynchronous**: the full-logbook `LIKE '%…%'` scan used to run synchronously on every keystroke. Typing is now debounced (250 ms) and the query runs in a worker — results are bound to their search generation, so superseded queries are discarded and typing stays responsive on large logs.
- **Bulk ADIF import and Wavelog download write through chunked transactions**: per-record autocommit cost one fsync per row and made large imports crawl. Both paths now batch 500 rows per transaction — duplicates inside one stream are still caught (the key lookup reads the uncommitted transaction), the Wavelog cursor still only advances through committed records, and aborts keep the previously imported rows.
- **Partner worked-DXCC panel is wired to its async fetch**: the heavy worked-summary query (18 statements per callsign) was flagged off the render path but never actually dispatched, leaving the panel permanently "NEW". The fetch now runs as a command, and a wanted-signature guard discards stale results for superseded callsigns.
- **Partner view no longer queries SQLite inside `View()`**: foreign-prefix DXCC resolution now uses a bounded, logbook-scoped country→DXCC memo loaded off the render path — misses are memoized too so an unknown country is not re-queried every frame.
- **DXC filter keys debounce the table rebuild** (180 ms): rapid cycling of the time/band/mode/continent filters coalesces into one query and one table build instead of one per keypress.
- **Editor paging is two statements again**: `COUNT(*) OVER()` made every page turn evaluate the whole filtered set; the page query and the total count are now separate, so page turns stay cheap on large logs.
- **Redundant `qsos` indexes dropped**: the plain `country` index (covered by the compound `(country, base_call)` index and the NOCASE index) and the consumerless `submode` index are removed on migration — every QSO save maintains fewer b-trees. Verified via query plans that no query lost index coverage.
- **Removed the unused 2048-slot `Events` channel from the WSJT-X listener** — it was allocated per listener and never read.
- **Worked panel queries use their indexes**: the DXCC scope's three-way `OR` predicate made SQLite abandon every index and full-scan the whole table in all six queries (a 34% win at 50k QSOs). It is now computed over a `UNION ALL` of two index-driven arms — the stored entity number (`idx_qsos_dxcc`) and the country-name fallback (`idx_qsos_country_nocase`, prefix included for "United States of America" variants) — with identical results. The grid scope's `LIKE 'DM03%'` full scan became a half-open range (`gridsquare >= 'DM03' AND < 'DM04'`) that seeks `idx_qsos_gridsquare` directly.
- **Worked-panel fetches coalesce**: a second fetch for the same signature already in flight is no longer dispatched — only a changed signature starts a new query, and the wanted-signature guard keeps discarding stale results.
...
- **Full SQLite audit for Raspberry Pi-class hardware**: every statement in the repository was inventoried against the schema and verified with query plans. The dashboard rate counters (3 full scans per refresh via `printf` on columns) became an index-served two-column range; the pending-sync backlog queries dropped their index-defeating `COALESCE` wrappers and gained a matching partial index; `psk_spots` gained a `flow_start` index so purge stops full-scanning a week of spots after every fetch; the APRS station listing gained a `(source, last_heard)` composite. Eight `qsos` indexes with no query consumer (`call`, `band`, `mode`, and five compound indexes) are dropped on migration — every QSO save now maintains ~13 b-trees instead of ~21. Dead store functions were removed, and new tests pin the rate-window semantics and the shipped indexes.
