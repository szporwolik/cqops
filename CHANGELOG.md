...
- **Stale-lock prompt is back**: the OS-lock refactor silently refused to start when the lock looked orphaned. A genuine live instance still fails fast, but when the recorded PID no longer exists — or was reused by an unrelated process (checked via the kernel's process name) — CQOps asks "Stale lock from PID … — delete it?" like before; on confirmation the file is removed and the lock re-acquired on a fresh inode.
