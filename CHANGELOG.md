...
### Fixes
- **PSK Reporter honors offline mode**: the `--offline` switch now blocks the F5 fetch, the auto-refresh, the tab cycling and the tab highlight — the panel shows "unavailable — offline mode" and never contacts the network. The QRZ Worked panel likewise drops the remote source and shows just "Worked · Local" while offline, the DX cluster never connects offline (F4 warns instead), and the status-bar Net dot turns yellow instead of claiming reachability.
