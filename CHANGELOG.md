...
- **Stale-lock prompt is back**: the OS-lock refactor silently refused to start when the lock looked orphaned. A live holder still fails fast ("another instance is running"), but when the recorded PID no longer exists CQOps asks "Stale lock from PID … — delete it?" like before; on confirmation the file is removed and the lock re-acquired on a fresh inode.
