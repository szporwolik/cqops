...
### Wavelog API v2
- <u>**Bearer authentication and scopes**</u>: the token travels only in the `Authorization` header; the connection test uses `/api/v2/status` plus `/api/v2/token` (whoami), and every v2 error (`invalid_token`, `token_expired`, `insufficient_scope`, `rate_limited`, `conflict`, …) maps to an actionable message.
- <u>**Typed lookups and station profiles**</u>: callsign lookup via `GET /api/v2/lookup?detail=full` with real booleans (no truthy-string guessing); station profiles come from `GET /api/v2/station`.
- <u>**Uploads and remote ids**</u>: bulk ADIF via `POST /api/v2/qso` with duplicate detection from the `parsed/imported/skipped` summary; single-QSO saves use the JSON create whose response carries the remote id (stored immediately, automatic ADIF fallback); batch and duplicate paths look ids up after upload. `wavelog_id` is the single source of truth — the legacy `wavelog_uploaded` column is dropped on migration, and pre-0.11 logs reconcile against the remote list (5000/page) so only genuinely new QSOs upload.
- <u>**Paginated download**</u>: `GET /api/v2/qso?format=adif&since_id=…` streams 250-row pages straight to disk (no memory buffering), strips later-page headers, advances the progress bar against `meta.total`, and aborts when the server stops advancing `lastfetchedid`.
- <u>**Edit sync**</u>: opening a synced QSO refreshes the form from `GET /api/v2/qso/{id}`, saving pushes a `PATCH /api/v2/qso/{id}` (local save always succeeds first), deleting also removes the remote copy — confirmations state the Wavelog effect, offline edits are kept pending.
- <u>**Live radio state**</u>: a `CQOps` radio is ensured on the radio resource and frequency, RX frequency, mode and power are pushed every 15 seconds (recreated automatically if deleted).
- **Station sync and a simpler wizard**: a selected Wavelog station profile mirrors grid, callsign, DXCC, zones and references into the logbook (best-effort, stale results discarded; Space over the Station ID field fills the whole form). The wizard lost its timezone step (system timezone is used), hides optional fields behind Ctrl+A, and every step — and every config form — ends with a visible `[ Save & Next ]` / `[ Save & Back ]` button.
- <u>**Shared club stations**</u>: a per-logbook option for a PC shared by several operators using the owner's `wl2_` key. Synced contacts become read-only — edits, deletes, purge and remote sync are blocked with a toast and no PATCH/DELETE is ever sent (changes stay on the Wavelog side). New QSOs upload normally and are attributed per operator (`OPERATOR` falls back to the station callsign when none is selected). The connection test requires `qso:write` and warns when the key also carries `qso:delete`; the saved key stays encrypted in the secrets store and is never shown again (leave the field empty to keep it, type to replace).

### Configuration
- **Unified save convention**: Integration, Callbook and Notifications menus now match General — Ctrl+S no longer saves and no longer appears in the help overlay or bottom bar. The in-form **[ Save & Back ]** button (**Enter**/**Space**) is the single save path. General, Logbooks, Rig, Contest and Operator forms also drop the redundant "Enter Save" help entry — the button is the advertised save path (Enter still saves).
...
- **Button feedback is toast-only**: config-menu test buttons no longer mix inline result lines with toasts. The GPS [Test] button in Integrations now reports through a toast (`GPS: connection verified` / failures) instead of a styled inline line under the button, and validation failures that used to be silently dropped — callbook test credentials, APRS test server/port/host, GPS test port/baud — now appear as toasts too. Dead inline result fields and the leftover QRZ test path in the Integrations menu were removed (callbook owns QRZ). The transient "…" dimming on buttons while a test runs stays as the only in-form progress cue.
- <u>**Unified form engine**</u>: every config menu, form, editor and the wizard now share one focus/navigation engine and shared row renderers — Tab/Shift+Tab, the Save & Back button, focus markers, hidden-row skipping and viewport scrolling in a single implementation. This fixes a whole class of focus bugs: double active markers, button focus leaking between forms, stray cursors and unreachable fields.

### Fixes
- **Menu cursor stays visible**: on small terminals the config menus (Integrations, Callbook, Contests, Logbooks, Rig) scrolled by a fractional mapping that could move the focus cursor out of the visible window. Every menu now clamps the viewport offset to the line carrying the focus marker (`scrollToFocusedLine`), so the cursor is always on screen — also across resize and hidden-row changes.
- **Edit Logbook navigation order**: Tab in Configuration → Logbooks → Edit Logbook visited Operator before Grid locator, out of sync with the rendered field order. The station form's engine row indices now match the render order (name, callsign, locator, GPS grid, IARU, continent, operator, …).
- **Integrations menu order matches the top pane**: the menu now lists APRS (F3), DX Cluster (F4) and PSK Reporter (F5) in the same order as the main-screen tabs, followed by the integrations without a pane (HTTP Server, GPS). Focus row numbering was renumbered to the new render order.
- **Main menu optimized**: Configuration (F9) now leads with station identity and operation — Logbooks, Operators, Rigs, Contests — then the online services (Integration, Callbook) and finally General. Notifications moved into General as a submenu row (`Notifications…`) — opening it keeps the General form alive and unsaved, and Back/Save returns to General. Digit quick-select (1–7) follows the new order, and the Integration entry's description reflects its updated internal order.
- **Save button spacing unified**: the `[ Save & Back ]` button now sits flush under the last content row in every config menu (General, Callbook, Notifications, Contests, Rigs, Operators, Logbooks — Integrations already was). The previous blank separator line is gone, and the rig/operator/logbook forms no longer change their button spacing depending on whether the last field was expanded.
- **Rig menu stray blank lines**: the Edit Rig form rendered empty lines after "Radio control:" and "Rotator control:" when the backend was None, and a blank between each selector row and its expanded sub-options (host/port/poll, UDP host/port). All rows now sit flush in every state — collapsed selectors have no gaps, expanded sub-options follow their selector directly. The same audit found the contest form's spacing shifted with its prefill toggles (a spacer lived outside the conditional block) and kept a blank between the checkbox and its exchange field; its layout is now identical in every toggle state with sub-fields flush and exactly one blank line before the marker reference section. The Integrations menu also dropped its blank separator lines between the DX Cluster, HTTP Server, GPS, APRS and PSK Reporter sections — sections now sit flush in both collapsed and expanded states (previously the APRS→PSK gap even differed between the two).
- **Stale-lock prompt is back**: the OS-lock refactor silently refused to start when the lock looked orphaned. A genuine live instance still fails fast ("another instance is running" — close it), but when the recorded PID no longer exists — or was reused by an unrelated process (checked via the kernel's process name) — CQOps asks "Stale lock from PID … — delete it?" like before; on confirmation the file is removed and the lock re-acquired on a fresh inode.
- **PSK Reporter honors offline mode**: the `--offline` switch now blocks the F5 fetch, the auto-refresh, the tab cycling and the tab highlight — the panel shows "unavailable — offline mode" and never contacts the network. The QRZ Worked panel likewise drops the remote source and shows just "Worked · Local" while offline, the DX cluster never connects offline (F4 warns instead), the status-bar Net dot turns yellow instead of claiming reachability, and callbook lookups run only local providers (logbook history + CTY.DAT) — QRZ/Callook/Wavelog are skipped. WSJT-X auto-logged QSOs also stay local: no enrichment upload is dispatched offline (WavelogID stays 0 until a later online sync). A full audit closed the remaining gates: Solar data never fetched, CTY/SCP/REF data files never refreshed, the REF rebuild (which downloads SOTA/POTA/WWFF/IOTA CSVs) never starts, the rapid internet re-check never fires, and the Integration/Callbook menus mirror offline so their Test Connection buttons refuse to run.
- <u>**Download identity capture**</u>: Wavelog serves the JSON QSO list newest-first but the ADIF export ascending by id, so per-page id sidecars could never align with the export pages — on a full download only ~one page of QSOs learned their remote ids and the result misleadingly reported the rest as "deferred". CQOps now captures the complete id list once and matches every exported row by verified identity; the result dialog distinguishes rows that failed to save from rows that were stored but still lack a remote link ("stored without remote link — will be linked on the next download"). The next download of an affected log reconciles the links and advances the cursor.
- <u>**Download honesty**</u>: "Wavelog is up to date" instead of a confusing zero result; the cursor freezes on failed inserts or unresolved identities (invalid records obey the same rule); remote ids are matched by verified identity (call/band/mode/date/time), never by list position.
- <u>**Upload honesty**</u>: snapshot and revision travel as a pair; id-attach retries use the accepted snapshot; a failed id write reports "accepted but not stored — will retry"; batch results carry sent/duplicate/failed/unresolved tallies; reconciliation ids survive batch failures; a save can no longer wipe a freshly assigned remote id.
- <u>**PATCH serialization**</u>: per-contact serialization lives on the model (survives F8 recreation), newer saves queue and coalesce behind the in-flight PATCH, and each chain is bound to its originating logbook/database/endpoint; CQ/ITU zones now sync, explicit nulls clear remote fields, and the dirty flag clears only for the acknowledged revision.
- <u>**Bulk pending-sync recovery**</u> (**Alt+P**): the backlog is rebuilt from the database and routed through the shared coordinator (one retry at a time, up to 50 rows), ack failures are tallied as unconfirmed instead of success, and empty results release the database lease honestly.
- <u>**Async safety**</u>: workers return immutable results applied on the owner loop; operations carry generations, logbook identity and database leases spanning the whole upload→retry→reconciliation chain (global handlers release every unconsumed lease); late results from replaced editors or previous logbooks are discarded; the callbook registry is rebuilt on every switch and station-sync completions run globally without clearing the in-progress exchange; completions are bound to their edit session and form revision.
- <u>**Message-pump integrity**</u>: one pending channel reader per operation (close can never fabricate a zero-count success), terminal messages bound to the operation id, pending lookups no longer swallow completions or progress, Abort keys reach the operation dialog, and the quit dialog overlays the source screen.
- **WSJT-X lifecycle**: both shutdown deadlocks fixed (mutex and queue drain); the unused 2048-slot Events channel removed; the pending-ADIF queue is capped at 64.
- **APRS lifecycle**: single-owner state, immutable worker snapshots, joined goroutines, and a scrubbed config copy for saves.
- **Dashboard**: stall-proof TCP classification, explicit TLS/HTTP listener shutdown, SSE stream termination with bounded write deadlines, HTML escaping of every log field and a stricter CSP.
- **DX Cluster**: one reconnect owner, UI recovery after drops, correct cache invalidation, path-line expiry, a fixed connect race and a continent-filter leak.
- **Rotor**: Extended Response Protocol (`+` prefix) and buffered line framing up to `RPRT` — split replies are reassembled and no longer corrupt the next command.
- **Single-instance lock**: an OS-backed exclusive lock (flock / LockFileEx) held for the process lifetime and released on initialization failure.
- **SQLite actually tuned now**: the DSNs switched to `_pragma` parameters — WAL, 5 s busy timeout and foreign keys really take effect on every pooled connection.
- **Editor correctness**: every field populates on open (stale numeric values cleared); remote refreshes can never overwrite local edits; completions never close a newer form or session; offline saves keep the remote id and stay pending; deleting a contact while a search filter is active clears the filter so the deleted row disappears from the list immediately; `base_call` and the prefix-derived DXCC stay consistent on callsign edits; upload normalization rewrites only the fields confirmed as mismatching.
- **UI consistency**: Recent QSOs appear right after downloads/imports, dashboard panels refresh after every mutation, the callsign info row keeps a fixed height (no layout shift), PSK Reporter stays silent on empty fetches, GPS fix loss and movement propagate (15 s expiry, per-logbook grid overrides), Backspace on the REF screen clears the whole search after a lookup (like the logbook editor search), the callbook menu renders every provider section on its own line (the QRZ.com and QRZ.RU headers were glued onto the previous priority field and invisible), and the wizard actually quits.
- **Config and privacy**: a broken `config.yaml` is reported and left untouched; a failed secrets save aborts the config write and clearing a credential deletes its secret, with retries persisting pending changes; reference-data downloads are bounded and asynchronous; a failed logbook switch leaves the previous database fully active; log files and the log directory are owner-only (0600/0700) with truncated grid precision.
- **Export fidelity**: ADIF export runs in a single read-only snapshot with a keyset cursor (no duplicated or missing rows during live logging) and includes DXCC; imports/exports always end in one honest terminal result, and exports are promoted from a temp file.
- <u>**Performance pass for Raspberry Pi-class hardware**</u>: debounced async editor search, debounced DXC table rebuild, two-statement paging, 500-row chunked import/download transactions, keyset-paginated upload preparation, the worked panel served by index-driven UNION ALL queries with fetch coalescing and no SQLite in `View()`, dashboard SSE marshalling once per event, lazy debug arguments, contest OR-filters and dashboard country checks on their own indexes, and a full SQLite audit — index-served rate counters, a partial pending-sync index, `psk_spots`/APRS composites, eight dead `qsos` indexes dropped (~13 b-trees per save instead of ~21) and dead code removed.
