...
- **Offline-first logging** — インターネット接続がなくてもローカルに QSO を記録できます。
...
- **任意の連携機能** — QRZ.com、Wavelog、DX Cluster、PSK Reporter、GPS、APRS、無線機制御、ローテーター制御、太陽活動データ、ブラウザー用 CQOps Live dashboard。
...
| Integrations | QRZ.com lookup の設定 |
...
QRZ.com lookup には、インターネット接続と QRZ XML subscription が必要です。
...
| Header 2 | title の下に表示する subtitle。未設定時は「Fast, portable ham radio logger」。 |
...
| Menu | Configures |
|---|---|
| General | Units、timezone、partner map/picture、solar panel、CTY.DAT/SCP/REF data source、Kitty Graphics、Debug mode |
...
| Integration | DX Cluster（host、port、login）、QRZ.com（username、password）、dashboard 用 HTTP Server（address、port、branding）、GPS service（serial/GPSD、grid precision） |
