...
| Kitty グラフィック端末 | [Kitty](https://sw.kovidgoyal.net/kitty/), [Ghostty](https://ghostty.org/), [WezTerm](https://wezfurlong.org/wezterm/) |
...
> ⚠️ **実験的機能.** 写真表示は Kitty 端末グラフィックプロトコルを使用でき、
> 互換性のある端末が必要です：Kitty, Ghostty, WezTerm。
> **F9 → General → Kitty Graphics** で有効にします。標準端末および
> グラフィック転送のない SSH セッションではグリフ写真にフォールバックします。
...
> ⚠️ **実験的機能.** Rotor control は実験的です。操作前にアンテナの物理的
> 限界を常に確認してください。**Alt+/** ですぐに移動を停止できるように
> してください。注意して使用してください — 誤った設定は rotor や antenna
> を損傷する可能性があります。
...
| Alt+; | elevation +5° |
| Alt+' | elevation −5° |
| Alt+\ | rotor を計算された path bearing へ向ける |
| Alt+/ | rotor を停止 |
...
> ⚠️ **実験的機能.** KISS TNC サポートは実験的です。運用に依存する前に
> 十分にテストしてください。
...
> ⚠️ **実験的機能.** KISS Server サポートは実験的です。運用に依存する前に
> 十分にテストしてください。
...
Integrations メニューでホストとポートを入力します：
...
| Alt+' | rotor elevation −5° |
| Alt+\ | 自局 grid から相手 grid への bearing に rotor を向ける |
| Alt+/ | rotor を停止 |
| Alt+0–9 | favorite を呼び出す |
