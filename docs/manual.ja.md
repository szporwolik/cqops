...
description: CQOpsのインストール、設定、使用方法の完全ガイド — 高速な端末ベースのアマチュア無線ロガー
---

> **注記:** この翻訳はLLMモデルを使用して生成されました。修正は歓迎します。`dev` ブランチへの Pull Request として送ってください。
...
CQOpsは高速な端末ベースのアマチュア無線ロガーです。シャック、山頂、フィールドデー、クラブ局など、スピード、信頼性、低システム負荷を必要とするオペレーター向けに設計されています。

**オフラインファースト。** ローカルQSOロギングはインターネットアクセスを必要としません。キャッシュされた参照データ、太陽データ、DXCCプリフィックスは一度ダウンロードされた後も利用可能です。Wavelog、QRZ.com、DX Cluster、PSK Reporterなどのネットワーク統合機能は接続が必要で、`--offline` モードではスキップされます。

**フィールド運用向けに構築。** CQOpsはQRP対応、SOTA/POTAフレンドリーで、Raspberry Piクラスのマシン、古いラップトップ、デスクトップ環境のないシステムでも快適に動作します。

**クラブ局対応。** CQOpsは複数のログブック、オペレータープロファイル、リグプリセットをサポートします。アクティブログブック、アクティブオペレーター、アクティブリグをワンキーで切り替えられます。

**ポータブル設計。** CQOpsはGoで書かれた単一バイナリです。CGO依存や必要なシステムサービスはありません。
...
CQOpsは完全なデスクトップロガーやWebベースのログブックプラットフォームの代替を意図していません。高速な端末ロギング、フィールド運用、オフライン使用、共有局ワークフローに焦点を当てています。
...
- 端末サイズ: 最低80×24文字。
- 推奨端末サイズ: 80×43以上。
- 最新の端末エミュレーターを推奨。WindowsではレガシーコンソールではなくWindows Terminalを使用してください。
...
初回起動時に、CQOpsは基本的な局設定のためのセットアップウィザードを開きます。ネットワーク統合機能はスキップできます。ローカルロギングはそれらなしで動作します。

1. **Station & Logbook** — 初期ログブック、局コールサイン、オペレーター、グリッドロケーターを設定。オプションでSOTA/POTA/WWFF参照、IARUリージョン、CQ/ITUゾーン、DXCC、SIG/SIG Info。Wavelog設定（URL、APIキー、局プロファイルID、Update、Test）もここで行えます。

2. **Rig** — リグプリセットを設定: 名前、モデル、アンテナ、電力、無線バックエンド。サポートされるバックエンド: None、flrig、Hamlib rigctld。オプションでHamlibローテーター制御とWSJT-X UDP統合。
...
すべてのウィザード設定は後で **F9** の設定メニューから変更できます。

---

## クイックスタート: 最初のQSOを記録

1. **CQOpsをインストールして実行。** プラットフォーム用のパッケージをダウンロードし、`cqops` を起動、少なくともコールサインとグリッドロケーターでセットアップウィザードを完了します。
...
ステータスバーにはCQOpsバージョン、アクティブログブック、アクティブリグ、局コールサイン、アクティブオペレーターが表示されます。右側には統合ステータスラベルとローカル時刻（`L`）およびUTC（`Z`）が表示されます。
...
| F2 | QRZ | Partner view: コールブックデータ、マップ、統計、写真 |
...
1. **自宅を出る前に**、インターネットアクセスがある状態でCQOpsを一度実行します。太陽データ、REFデータ、DXCCプリフィックスなどのキャッシュが生成されます。
2. **オフラインになる前にキャッシュを確認。** ソーラーパネルにデータが表示され、**F6** のREF検索が結果を返すことを確認します。
...
2. **アクティブオペレーターを切り替え:** QSO formで **Ctrl+O** を押します。アクティブオペレーターがステータスバーに表示され、保存されたQSOの `OPERATOR` フィールドに書き込まれます。
...
共有局で保存する前に、ステータスバーのアクティブログブックとアクティブオペレーターを確認してください。
...
2. **アクティブログブックを切り替え:** QSO formで **Ctrl+L** を押します。ステータスバーにアクティブログブックが表示されます。
...
Wavelogアップロードが失敗した場合、QSOはローカルログブックに残り、後で再試行できます。ログブックをパージするとフェッチIDが `0` にリセットされ、完全な再ダウンロードが可能になります。
...
⚠️ 交換フィールドはコンテストがアクティブな場合のみ表示されます。**(▼)** のフィールドは **PgUp/PgDn** で切り替えます。
...
パス/方位線は両方のグリッドロケーターが判明している場合に距離と方位角を表示します。**DUPE!**、**New Call!**、**New DXCC!** などのバッジも表示されます。
...
コンテストはQSO formに交換フィールドとシリアル処理を追加します。

Logbook Editor（**F8**）で **Ins** を押してコンテストを作成または設定します。コンテスト名、日付、ADIFコンテストID、交換テンプレートを設定します。
...
バンドプランブラウザ（**F7**）はアマチュアバンド、VHF/UHF範囲、CB、PMR446、ブロードキャストプリセットへのクイックアクセスを提供します。選択した周波数でアクティブリグをチューニングできます。バンドプランデータはMarkdownとしてエクスポートも可能です。
...
太陽データにはSFI、黒点数、A/K指数、hamqsl.comからのバンド別状況が含まれます。ライブ更新にはインターネットアクセスが必要です。キャッシュされたデータは取得成功後オフラインで利用可能です。
...
機密認証情報は同じ設定ディレクトリの `secrets.enc` に別途保存されます。シークレットはマシンに紐付いたキーで暗号化されるため、設定を別のマシンに移動する際は認証情報を再入力する必要があります。
