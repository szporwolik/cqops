...
- **可选集成功能** — 多提供商呼号数据库（QRZ.com、HamQTH、QRZ.RU、Callook.info）、Wavelog、DX Cluster、PSK Reporter、GPS、APRS、电台控制、天线旋转器控制、太阳活动数据和 CQOps Live 浏览器仪表板。
...
也可通过 `pacaur`、`aura` 或手动 `makepkg` 安装。PKGBUILD 位于 [aur.archlinux.org/packages/cqops-bin](https://aur.archlinux.org/packages/cqops-bin)。
...
| `Integrations` | 呼号查询设置（QRZ.com、HamQTH、QRZ.RU、Callook.info） |
| `General` | IANA 时区 |
| `Summary` | 检查并保存 |
...
| 呼号数据库（QRZ.com / HamQTH / QRZ.RU / Callook.info） | 姓名、QTH、网格、国家、CQ 区、ITU 区、DXCC、大洲、照片 |
...
### 呼号数据库（QRZ.com、HamQTH、QRZ.RU、Callook.info）
...
3. **QRZ.RU** — 面向俄罗斯及周边国家的免费服务。需要 API 登录。返回姓名、QTH、网格、经纬度、等级、LoTW/eQSL 状态和照片。
4. **Callook.info** — 面向美国呼号的免费服务。无需账户，FCC 查询速度快。
...
| Callbook | QRZ.com、HamQTH、QRZ.RU、Callook.info 提供商；优先级排序、基础呼号回退、Wavelog 查询 |
