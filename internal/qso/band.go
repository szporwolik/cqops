...
// bandRanges are the widest amateur band limits (used for DeriveBand).
// For region-specific limits, use regionBands().
...
// regionBands returns the amateur band limits for the given IARU region.
// Region 0 maps to Region 2 (the widest limits). Differences from the
// default bandRanges are: 160m lower edge, 80m upper edge, 40m upper edge.
func regionBands(region int) []bandRange {
	switch region {
	case 1:
		return []bandRange{
			{0.1357, 0.1378, "2190m"},
			{0.472, 0.479, "630m"},
			{0.501, 0.504, "560m"},
			{1.81, 2.0, "160m"}, // R1: 1810-2000 kHz
			{3.5, 3.8, "80m"},   // R1: 3500-3800 kHz
			{5.06, 5.45, "60m"},
			{7.0, 7.2, "40m"}, // R1: 7000-7200 kHz
			{10.1, 10.15, "30m"},
			{14.0, 14.35, "20m"},
			{18.068, 18.168, "17m"},
			{21.0, 21.45, "15m"},
	...
	case 3:
		return []bandRange{
			...
			{0.501, 0.504, "560m"},
			{1.8, 2.0, "160m"},
			{3.5, 3.9, "80m"}, // R3: 3500-3900 kHz
			{5.06, 5.45, "60m"},
			{7.0, 7.2, "40m"}, // R3: 7000-7200 kHz
			{10.1, 10.15, "30m"},
			{14.0, 14.35, "20m"},
			{18.068, 18.168, "17m"},
			{21.0, 21.45, "15m"},
	...
	// Default / Region 2 (widest limits, same as original bandRanges).
	return bandRanges
}
...
func IsInHamBand(freqMHz float64, region int) bool {
	for _, r := range regionBands(region) {
		if freqMHz >= r.low && freqMHz <= r.high {
			return true
		}
	}
