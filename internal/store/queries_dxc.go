...
func InsertDXCSpots(db *sql.DB, spots []DXCSpot) (int, error) {
	...
	stmt, err := tx.Prepare(`INSERT OR REPLACE INTO dxc_spots
		(dx_call, frequency, band, mode, mode_cat, comment, spotter, dx_cont, spot_cont, received_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
	if err != nil {
	...
	inserted := 0
	for _, s := range spots {
		res, err := stmt.Exec(s.DXCall, s.Frequency, s.Band, s.Mode, s.ModeCat, s.Comment, s.Spotter, s.DXCont, s.SpotCont, s.ReceivedAt)
		if err != nil {
...
func QueryDXCSpots(db *sql.DB) ([]DXCSpot, error) {
	rows, err := db.Query(`SELECT id, dx_call, frequency, band, mode, mode_cat, comment, spotter, dx_cont, spot_cont, received_at
	...
	for rows.Next() {
		var s DXCSpot
		if err := rows.Scan(&s.ID, &s.DXCall, &s.Frequency, &s.Band, &s.Mode, &s.ModeCat, &s.Comment, &s.Spotter, &s.DXCont, &s.SpotCont, &s.ReceivedAt); err != nil {
...
func QueryDXCSpotByCall(db *sql.DB, call string) (*DXCSpot, error) {
	var s DXCSpot
	err := db.QueryRow(`SELECT id, dx_call, frequency, band, mode, mode_cat, comment, spotter, dx_cont, spot_cont, received_at
		FROM dxc_spots WHERE dx_call = ? ORDER BY received_at DESC LIMIT 1`, call).Scan(
		&s.ID, &s.DXCall, &s.Frequency, &s.Band, &s.Mode, &s.ModeCat, &s.Comment, &s.Spotter, &s.DXCont, &s.SpotCont, &s.ReceivedAt)
	if err != nil {
